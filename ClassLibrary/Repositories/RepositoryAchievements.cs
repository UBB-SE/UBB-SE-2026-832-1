...
public sealed class RepositoryAchievements(AppDbContext dbContext) : IRepositoryAchievements
{
    ...
    public async Task<int> GetConsecutiveWorkoutDayStreakAsync(int clientId, CancellationToken cancellationToken = default)
    {
        ...
        // NON-REPOSITORY LOGIC: the streak computation below is a business algorithm,
        // not data access. It is kept here to preserve the existing public contract;
        // ideally it should be moved to an AchievementService that calls a repo method
        // returning the raw list of distinct workout dates.
        if (dates.Count == 0)
        {
            return 0;
        }
    ...
    public async Task<IReadOnlyList<AchievementShowcaseItem>> GetAchievementShowcaseForClientAsync(int clientId, CancellationToken cancellationToken = default)
    {
        ...
        // Defensive de-duplication by title (case-insensitive) — preserves
        // the behavior of the original SQL implementation.
        return items
