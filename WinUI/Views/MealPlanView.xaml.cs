...
public sealed partial class MealPlanView : Page
{
    ...
    private async Task ShowSettingsDialog(UserDataDto userData)
    {
    ...
    private async void SaveToDailyLogButton_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            StatusMessageText.Text = $"All {this.viewModel.GeneratedMeals.Count} meals saved to daily log!";
        }
        catch (Exception ex)
        {
            await this.ShowSimpleDialog(TitleSaveFailed, $"Failed to save to daily log:\n\n{ex.Message}");
        }
    }
    ...
    private async void AddMealToLogsButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            await this.mealPlanService.SaveMealToDailyLogAsync(meal.Id, meal.Calories, this.userSession.CurrentClientId);
    ...
    {
        await this.ShowSimpleDialog(this.viewModel.ErrorDialogTitle, this.viewModel.ErrorDialogMessage);
